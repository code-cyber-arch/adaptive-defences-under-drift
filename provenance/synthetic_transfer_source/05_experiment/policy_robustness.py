...
def enumerated_arms(detector="adwin"):
    return [{'name': 'map_' + ''.join(bits), 'policy': 'rl', 'detector': detector,
             'table': policy_table(''.join(bits)), 'variant': 'alarm_persistence'}
            for bits in product('01', repeat=4)]


def comparison_arms(selected, models, detector):
...
def run_one(job):
    ...
    folder = output / arm['detector'] / condition['key'] / arm['name'] / f'feedback_{fraction:.2f}_delay_{delay}'
