...
def verify():
    ...
    assert len(models)==len(expected)==72
    ...
    for m in models:
        ...
        assert record['request_sha256']==p.sha(source/'request.json')
    ...
    values=pd.DataFrame(rows); assert len(values)==6024
    ...
    for stage,count in [('validation',1344),('evaluation',2440),('sensitivity',2688)]:
    ...
    p.write(dest/'verification.json',{'status':'passed','trained_controllers':72,'comparison_executions':6024,
...
def report():
    ...
    labels=['Confirmed reset','Error rule','Selected mapping','2-state Q / 7','2-state Q / 17','2-state Q / 27',
    ...
    selected_arms=['no_reset','confirmed','error_rule','selected_map','q_alarm_persistence_7','q_alarm_persistence_17','q_alarm_persistence_27']
    for ds in cfg['datasets']:
    ...
    p.write(OUT/'completion.json',{'status':'complete','datasets':cfg['datasets'],'controllers':72,
                                 'comparison_executions':6024,'completed_utc':p.now(),'protocol_sha256':p.sha(OUT/'protocol.json')})
