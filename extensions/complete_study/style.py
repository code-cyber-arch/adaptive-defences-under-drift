"""Report figure layout derived from the established compact four-panel style."""
...
def draw(data, specs, title, subtitle, series, destination, note, styles=None, limits=None, labels=None, book=None):
    ...
    for ax, (metric, label, scale) in zip(axes.flat, specs):
        ...
        if finite_values:
            ...
            for category in set(range(len(labels)))-available_categories:
        ...
        ax.set_xticks(range(len(labels)), labels, rotation=38 if len(labels)>3 else 0, ha='right' if len(labels)>3 else 'center')
        ...
        for left in ([.5,4.5] if len(labels)==7 else []):
    ...
    if book is not None:
        fig.text(.085,.015,title,fontsize=7,color='#475569')
        book.savefig(fig)
    plt.close(fig)
