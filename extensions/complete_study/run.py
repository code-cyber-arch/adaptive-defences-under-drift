...
def run(args):
    ...
    with (out/'.full_study.lock').open('a') as lock:
        ...
        if not args.pilot:
            ...
            p.write(out/'full_status.json',dict(status='running',phase='complete_feedback',utc=p.now()))
            feedback.run(out,sources,configs,conditions,detectors,args.workers)
        ...
        if not args.pilot:
            ...
            p.write(out/'full_status.json',dict(status='running',phase='comparison_matrix',utc=p.now()))
            comparisons.build(out,sources,configs,conditions,detectors,args.workers)
        completion=p.read(out/'full_completion.json')
        completion.update(status='complete',all_comparisons_verified=not args.pilot,utc=p.now())
        if not args.pilot:
            completion.update(comparison_verification_sha256=p.sha(out/'analysis/comparisons/verification.json'),
                              full_feedback_verification_sha256=p.sha(out/'analysis/full_feedback/verification.json'))
        p.write(out/'full_completion.json',completion)
