...
def verify_and_report(out,sources,configs,conditions,seeds,detectors,pilot):
    ...
    if not pilot:
        from .balance import publish_comparison_entries
        publish_comparison_entries(out)
    render(out,frames,conditions,seeds,pilot)
    from .figures import render as render_report_figures
    render_report_figures(out,pilot=pilot)
    p.write(out/'full_report_manifest.json',{'files':{file.relative_to(p.ROOT).as_posix():p.sha(file) for file in [*(out/'analysis/figures').glob('*'),*(out/'analysis/full_study').glob('*'),*([p.ROOT/'07_documentation/RESULTS.md'] if not pilot else [])]},'utc':p.now()})
    p.write(out/'full_completion.json',dict(status='complete' if pilot else 'comparisons_pending',pilot=pilot,verification=verification,protocol_sha256=p.sha(out/'full_protocol.json'),utc=p.now()))
    ...
    for ds in ['SEA_A','RBF_I','radar']:
        ...
        for base_key in {c['base_key'] for c in selected}:
            ...
            for c in group:
                ...
                y=truth.host_reference_label.to_numpy()[mask];z=pred.prediction.to_numpy()[mask]
                precision=[];recall=[]
                for label in c['classes']:
                    tp=int(((y==label)&(z==label)).sum());fp=int(((y!=label)&(z==label)).sum());fn=int(((y==label)&(z!=label)).sum())
                    precision.append(tp/(tp+fp) if tp+fp else 0.);recall.append(tp/(tp+fn) if tp+fn else 0.)
                rows.append(w.metadata(c)|{k:scores[k] for k in ['host_n','host_accuracy','host_macro_f1']}|{'macro_precision':float(np.mean(precision)),'macro_recall':float(np.mean(recall))})
...
def render(out,frames,conditions,seeds,pilot):
    ...
    if not pilot:
        import shutil
        archive=p.ROOT/'provenance/report_exports'/p.sha(out/'protocol.json')
        for file in [out/'report_manifest.json',*(out/'analysis/figures').glob('*')]:
            if file.is_file():
        ...
        snapshot=out/'analysis/full_study/rl_report.md'
        if not snapshot.exists():snapshot.write_bytes(rl_path.read_bytes())
        rl_text=snapshot.read_text().replace('All 11,680 comparison executions passed trace checks: 1,344 validation, 5,656 default-setting evaluation and 4,680 additional feedback runs. The sensitivity table has 5,616 entries including 936 shared defaults.', 'All 35,280 RL/control comparison executions passed trace checks: 1,344 validation, 5,656 default-setting evaluation and 28,280 additional feedback runs. The sensitivity table has 33,936 entries including 5,656 shared defaults.').replace('on clean and severe label poisoning for every dataset and detector', 'on all seven clean/poisoning conditions for every dataset and detector').replace('Experiment 3 retains its existing three-dataset evidence.','Experiment 3 uses the same evaluation partitions and five-seed grid.')
        overview=['# Complete five-seed experimental results','',
                  'All three experiments use the same five-by-seven comparison structure: 35 comparison entries each for SEA, RBF and RADAR. These refer to 35, 35 and 31 unique input conditions respectively; RADAR shares its clean reference across repetitions. Conditions receive equal weight within each dataset. RQ1 and RQ2 remain unchanged.','',
                  ...
                  '- [Cross-experiment comparison tables](../results/per_dataset_study/analysis/comparisons/cross_experiment_pairs.csv)',
