...
from common.replication import training_request_matches
...
def train_one(root, config, seed, variant='alarm', detector='adwin'):
    ...
    if manifest.get('data_role') != 'training' or manifest.get('dataset') != dataset:
        raise ValueError('RL training requires a declared training partition of the matching dataset')
    for c in manifest['conditions'].values():
        if c['stream'] != dataset or (dataset != 'radar' and c['base_seed'] not in config['base_seeds']):
            raise ValueError('RL training contains another dataset or split')
        if dataset == 'radar' and c.get('source_partition', {}).get('role') != 'training':
            raise ValueError('RADAR fitting requires a training-only chronological partition')
    if output.exists():
        saved = p.read(output)
        if not training_request_matches(saved['request_sha256'], root):
            raise ValueError('RL training contract changed')
        if saved['variant'] != variant or saved.get('detector') != detector or saved.get('dataset') != dataset:
        ...
        return {'seed': seed, 'variant': variant, 'detector': detector, 'dataset': dataset, 'path': output.relative_to(p.ROOT).as_posix(), 'sha256': p.sha(output)}
    ...
    with threadpool_limits(limits=1):
        for epoch in range(config['rl']['epochs']):
            ...
            for position in rng.permutation(len(jobs)):
                ...
                transitions.extend((dict(t, epoch=epoch, condition=c['key'], detector=detector, dataset=dataset) for t in new))
                rows.append({'epoch': epoch, 'condition': c['key'], 'seed': seed, 'detector': detector, 'dataset': dataset, 'reward': sum((t['reward'] for t in new)), 'training_visits': stats['training_visits'], 'resets': stats['resets'], 'states': len(agent.table), 'epsilon': agent.epsilon})
                print(f"RL {dataset}/{detector}/{variant}, seed {seed}: epoch {epoch + 1}/{config['rl']['epochs']}, {c['key']}, states {len(agent.table)}", flush=True)
    ...
    record.update(detector=detector, dataset=dataset, request_sha256=p.sha(root / 'request.json'), training_base_seeds=config['base_seeds'], training_conditions=list(manifest['conditions']), files={f.name: p.sha(f) for f in [output.parent / 'episodes.csv', output.parent / 'transitions.parquet', output.parent / 'policy.csv']})
...
def train(root, config, workers):
    ...
    p.write(root / '04_rl_training/manifest.json', {'models': models, 'evaluation_updates': False,
            'dataset': config['dataset'], 'detectors': detectors, 'models_per_detector': len(variants) * len(config['rl']['seeds'])})
