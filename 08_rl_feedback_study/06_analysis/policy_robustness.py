...
METRICS = ['host_macro_f1', 'host_accuracy', 'poison_admission_rate',
           'clean_withhold_rate', 'resets', 'training_visits', 'seconds', 'malicious_recall', 'benign_fpr']
...
def report(records, folder):
    ...
    keys = ['detector', 'condition', 'feedback_fraction', 'delay_blocks']
    ...
    group = ['detector', 'stream', 'mode', 'level', 'arm', 'feedback_fraction', 'delay_blocks']
    ...
    if len(baseline):
        feedback = values.merge(baseline[['detector', 'condition', 'arm'] + METRICS],
                                on=['detector', 'condition', 'arm'], suffixes=('', '_baseline'), validate='many_to_one')
        for metric in METRICS:
    ...
    condition_means = values.groupby(['stream', 'detector', 'arm', 'mode', 'level', 'feedback_fraction', 'delay_blocks'])[METRICS].mean()
    condition_means.to_csv(folder / 'condition_means.csv')
    condition_means.groupby(['stream', 'detector', 'arm', 'feedback_fraction', 'delay_blocks']).mean().to_csv(folder / 'detector_policy_summary.csv')
    p.write(folder / 'manifest.json', {'runs': len(values), 'inference': 'Descriptive: synthetic stream seeds and conditional attack assignments on one RADAR stream are different replication units. Grand means weight conditions equally; RL seeds and repeated no-reset controls are not independent datasets.',
