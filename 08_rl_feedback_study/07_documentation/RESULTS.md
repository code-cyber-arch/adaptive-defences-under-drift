# Complete five-seed experimental results
...
The study contains 1,818 fixed-response configurations, 404 passive detector evaluations, 120 RL controllers, 30 fitted filters and 10,403 filter/control configurations. Shared no-reset references and reused trajectories are not independent replications.
...
- [Complete-study verification](../results/per_dataset_study/analysis/full_study/verification.json)

# Detector and reset-policy comparison across all datasets

RQ1 and RQ2 remain unchanged. These controls and robustness analyses belong to Experiments 1 and 2. Experiment 3 uses the same evaluation partitions and five-seed grid.
...
All 35,280 RL/control comparison executions passed trace checks: 1,344 validation, 5,656 default-setting evaluation and 28,280 additional feedback runs. The sensitivity table has 33,936 entries including 5,656 shared defaults. Training contains 3,360 episodes across 120 controllers. Repeated no-reset controls are not independent detector evidence.
...
Sensitivity varies usable feedback (1%, 2%, 5%) and delay (one or five blocks) on all seven clean/poisoning conditions for every dataset and detector, retaining the same 5% reserve. Controllers stay frozen. RADAR malicious recall and benign false-positive rate are retained in the metric tables.
