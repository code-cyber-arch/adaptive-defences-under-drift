"""Extension engine: filter before monitoring/training, score all eligible host rows.

Derived from the frozen 05_experiment engine. The original benchmark is unchanged.
The no-filter path is regression-tested against that engine.
"""
...
def execute(observations, audits, classes, policy, guarded, config, controller=None, withheld=None):
    ...
    withheld = np.zeros(n, dtype=bool) if withheld is None else np.asarray(withheld)
    if withheld.dtype != bool or withheld.shape != (n,):
        raise ValueError("Withholding must be a Boolean vector aligned to source rows")
    if withheld[:warmup].any() or withheld[reserved].any():
        raise ValueError("Filter must preserve warmup and protected reservations")
    ...
    for block, left in enumerate(range(0, n, block_size)):
        ...
        kept_ids = row_ids[~withheld[row_ids]]
        kept_features = [x for i, x in zip(row_ids, features) if not withheld[i]]
        blocks[block] = (kept_ids, kept_features, labels[kept_ids])
        ...
        error = float(np.mean(prediction[kept_ids] != labels[kept_ids])) if len(kept_ids) else float("nan")
        fired = False
        requested = None
        response_event = 'warmup'
        if left >= warmup:
            if len(kept_ids):
                fired = detectors.update(detector, prediction[kept_ids] != labels[kept_ids], values[kept_ids])
        ...
        events.append({'rl_action': rl_action, 'rl_state': rl_state, 'rl_reward_delivered': delivered_reward, 'block': block, 'start_row': left, 'end_row': right, 'scored': left >= warmup, 'feedback_error': error, 'monitor_rows': len(kept_ids) if left >= warmup else 0, 'filtered_rows': int(withheld[left:right].sum()), 'raw_fire': fired, 'response_event': response_event, 'reset_requested': bool(requested), 'reset_committed': bool(requested and accepted), 'training_blocks': json.dumps(proposed_blocks), 'candidate_rows': len(train_ids), 'accepted': accepted, 'audit_latest_row': latest_audit, 'candidate_seconds': candidate_seconds, 'predict_seconds': predict_seconds, 'gate_seconds': gate_seconds, **{k: v for k, v in gate.items() if k != 'supported_classes'}})
        if controller is not None:
    ...
    result = pd.DataFrame({'row_id': np.arange(n), 'prediction': prediction, 'audit_reserved': reserved, 'filter_withheld': withheld, 'scored': (np.arange(n) >= warmup) & ~reserved, 'first_admitted_block': admitted, 'candidate_training_visits': attempt_visits, 'accepted_training_visits': accepted_visits})
    ...
    if (attempt_visits[withheld] > 0).any():
        raise AssertionError('Filtered rows reached candidate training')
    stats = {'filter_rows_withheld': int(withheld.sum()), 'rows': n, 'training_visits': train_total, 'accepted_training_visits': accepted_total, 'accepted_refit_rows': refit_total, 'resets': actual_resets, 'seconds': time.perf_counter() - clock_start, 'pending_candidate': response.pending is not None, 'audit_rows_reserved': int(reserved.sum()), 'policy': policy, 'guarded': guarded}
