"""Fit filters on the shared disjoint dataset partitions and declared seed."""
import pickle
import time
...
from extensions.filter_study import protocol as s
...
def collect(root, stream, role, config):
    ...
    for meta in manifest['conditions'].values():
        ...
        if meta.get('source_partition', {}).get('role', role) != role:
            raise ValueError('Filter input belongs to another chronological partition')
...
def fit_stream(out, roots, stream, config, settings):
    ...
    source_roles = ['training', 'validation']
    candidates = {0: [], 1: []}
    hashes = {0: [], 1: []}
    classes = set()
    feature_names = None
    ...
    record = {'stream': stream, 'seed':settings['seed'], 'fit_rows': len(train), 'calibration_rows': len(calibration),
