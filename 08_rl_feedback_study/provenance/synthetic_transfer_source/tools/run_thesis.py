...
def main():
    ...
    c = deepcopy(base)
    c.pop('input_bank')
    c.update(name='training', persist_inputs=True, streams=['SEA_A', 'RBF_I'], rows=base['development']['rows'], base_seeds=[base['development']['training_seed']], detectors=base['detectors'], purpose='Matched fitting of alarm-only and persistence RL on separate synthetic development data')
