...
**The complete five-seed experiment covers SEA, RBF and RADAR with all four detectors.** The study covers Experiments 1–3 under the existing RQ1 and RQ2. Baselines, passive detectors, fixed screened/unscreened policies, RL and filtering share the five-seed evaluation grid.
...
The current study is organised in the main `00_streams`–`07_documentation` folders. [Publication scope](docs/PUBLICATION.md) explains the retained public evidence and excluded local artifacts. [Thesis evidence](07_documentation/thesis_evidence/README.md) contains the report figures and supporting summaries. Cite the exact repository commit using the author and title in `CITATION.cff`.

```sh
python3 scripts/check_publication.py
```

Historical execution records and the retained earlier implementation are isolated under `provenance/`; they are not alternative current study instructions. Large input and prediction Parquet files remain local.
